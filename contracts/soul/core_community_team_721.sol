...
contract SoulmetaTeamNFT is DivestorUpgradeable, ERC721EnumerableUpgradeable{
    ...
    function newTeamBatch(address[] calldata leaders_, uint toMeCount_) public onlyOwner returns (bool) {
        for (uint i = 0; i < leaders_.length; i++) {
            newTeam(leaders_[i], toMeCount_);
        }
        return true;
    }

    function newTeam(address leader_, uint toMeCount_) public onlyOwner returns (bool) {
